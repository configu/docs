...
export const introStoreName = 'Google Secret Manager';
export const scheme = 'gcp-secret-manager';
export const uriStructure = '[path to keyfile][?projectId=]';
export const uriExample = 'service-account.json?projectId=my-project';
...
```node.js
{{gcp-secret-manager://<projectId>@[set/]<schema>[.key][?key=[key]]}}
```

Example:

```node.js
{{gcp-secret-manager://my-project@example-set/example-schema.foo}}
```
