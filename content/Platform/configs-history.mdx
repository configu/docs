...
- Only members with the `Admin` role have permission to access the `show diff` and `revert` actions.
...
To evaluate using the CLI, make sure to set the `CONFIGU_TAG` environment variable to the tag name you wish to use or provide the `tag` parameter directly [via a `.configu` file](../cli-config).
