...
### Feature settings
- **Config Masking** - While we secure every value we receive, masked visibility helps teams to increase their security. When the Config Masking feature is on (by default), values are not displayed in the dashboard until there is an intent performed by the user to reveal the value.
    - Unmasked - Always displayed - does not require an intention to reveal
...
Note: Configu orchestrator is an open-source and free forever
