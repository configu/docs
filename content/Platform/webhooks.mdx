...
Webhooks enable changes in Configu to be integrated into your other tools like CI/CD or messaging platforms.
For example, a webhook can trigger a Github action flow when a Config is changed or notify about it on a Slack channel.

## Creating a Webhook

1. Navigate to the **Webhooks** page under the **Integrations** tab
...
### Customizing the webhook events
...
- `appUrl` - Link to the organization page within the Configu SaaS
...
### Customizing the webhook headers

The webhook headers and payload may be customized for specific use cases where you need to add properties to the webhook invocation.
...
To verifies that the webhook works, press on the **Test Webhook** button under the three dots menu.

![image](./img/webhooks/webhook-test.png)

## Disabling or Removing a Webhook

To enable/disable a webhook by clicking the Status button.
...
- Communication integrations
  ...
  - Gmail
- CI/CD integrations
  - [Github Action](/webhooks-github-action)
  - GitLab CI/CD
  - Circle CI
- Others
  - Zapier