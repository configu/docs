...
1. **Version control:** Configurations can be versioned alongside the code, allowing you to always match between them, rollback to previous versions, and have a historical record of all modifications.

2. **Reproducibility:** Configurations become part of the deployment pipeline, ensuring that the same configuration is used consistently across different environments or deployments without the need for duplicating and synchronizing every change you made for each environment.
...
The success and popularity of these tools are proof that the developers prefer not to switch context or jungle between several tools and can "code" or at least "as-a-code" any task inside the IDE, especially configurations. 
