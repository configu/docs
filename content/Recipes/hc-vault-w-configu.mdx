...
and Hashi Corp Vault is a great opportunity to show how Configu lets you only worry about your config
[schemas](/schemas), with Configu providing the rest of what's needed, including talking
to Vault's API to get your new config values in place. Check out the tutorial below

In order to complete the tutorial, you'll need a Hashi Corp Vault account (or an on-prem version), Git, Configu's CLI,
...
we will create a CFGU schema declaration for this service so for every change that we will make we will need to change only the key only on the schema and initialize values using the same interface.
...
Although saving configurations in the source control is considered to be bad practice, the CFGU format is designed to be part of the code since it shouldn't include any sensitive values.
Doing that increases the developers' velocity and helps them to avoid leaving the terminal/IDE for other config management platforms.
...
```bash
configu export --schema "./my-app.cfgu.json" --defaults | configu export --run "py my-app.py"
```

- Inject the variables into your shell
```bash
configu export --schema "./my-app.cfgu.json" --defaults | configu export --source
```

- Download and use .env file or any other format you want
```bash
configu export --schema "./my-app.cfgu.json" --defaults | configu export --format "Dotenv" > .env.development
```

## Step 3 - Manage configs in Hashi Corp Vault using Configu Orchestrator

Using a single set of commands we can control any store from local files on git to secret managers.
In the following example, we will manage our configs over our Hashi Corp Vault secret manage.
