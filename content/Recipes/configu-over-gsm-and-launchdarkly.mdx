...
[Google Secret manager (GSM)](https://cloud.google.com/secret-manager) and [LaunchDarkly](https://apidocs.launchdarkly.com/tag/Feature-flags).

![image](./img/gsm-launch-darkly-diagram.png)
...
## Step 3 - Manage configs in GSM and LaunchDarkly using Configu Orchestrator

Using a single set of commands we can control any store from local files to secret managers.
In the following example, we will manage our configs over Google Secret Manager (GSM) and LaunchDarkly.

### Authenticate GSM and LaunchDarkly

For GSM, Configu's CLI needs to be directed to a [service account key file](https://cloud.google.com/iam/docs/service-accounts-create) by providing a file path and a project id via the [.configu file](../cli-config).
...
```bash
configu upsert --store "my-gsm" --set "prod" --schema "./app.cfgu.json" \
    -c "ENABLE_CONTANT_FORM=true" \
```

```bash
configu upsert --store "my-launchdarkly" --set "prod" --schema "./app.cfgu.json" \
...
```bash
configu upsert --store "my-gsm" --set "test" --schema "./app.cfgu.json" \
    -c "ENABLE_CONTANT_FORM=false" \
&& configu upsert --store "my-launchdarkly" --set "test" --schema "./app.cfgu.json" \
...
```bash
configu eval --store "my-gsm" --set "prod" --schema "./app.cfgu.json" \
 | configu eval --store "my-launchdarkly" --set "prod" --schema "./app.cfgu.json" \
