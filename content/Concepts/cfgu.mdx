...
- **lazy**: Specifies that a value for a given configuration key should only be set during the [eval command](../commands/#eval-and-export) execution, either using the `configs` parameter or the `--config` flag in the CLI.
  - This property is optional.
