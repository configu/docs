...
A [configuration key](https://github.com/configu/configu/blob/main/types/ConfigSchema.ts#L21) is the name of the configuration as it appears in the code. It is used to identify and reference a specific configuration, and can be used to access the value of the configuration in the code.
...
[The Cfgu format](https://github.com/configu/configu/blob/main/types/Cfgu.ts) consists of a set of properties that specify important characteristics of a software configuration. Here is a list of the properties:

<DefinitionList>

`type` [Required]
: The type of data that the configuration represents, such as `String`, `Number`, `Boolean`, [etc](/cfgu-types).

`pattern` [Optional, type=Regex]
: A regular expression that the configuration value must match.

`default` [Optional]
: A default value for the configuration, in case it is not provided.

`required` [Optional]
: Whether the configuration is required for the application to function properly. If set to true, the configuration must be provided.

`depends` [Optional]
: A list of other configurations that the current configuration depends on.

`template` [Optional, type=String]
: A string containing Mustache templates that can be used to construct a templated value from other configuration values.

`description` [Optional]
: A description of the configuration's role in the code.
