...
You can then pass the environment variable to the CLI to replace the mustache template with the environment variable value. Example [eval command](../cli-commands#configu-eval) with an environment variable and the example schema above:

<CodeTabs labels={['Unix/Linux/MacOS','Windows']}>
```bash
PASSWORD=somePassword configu eval --store store1 --schema <value> [--set <value>] [-c <value>]
```
```bash
set PASSWORD=somePassword && configu eval --store store1 --schema <value> [--set <value>] [-c <value>]
```
</CodeTabs>
