...
manage [ConfigStore](/docs/config-store) connection-string as labels that can later be used as values for the `--store` flag in other commands.
...
```shell
store=configu;org=<org>;token=<token>;type=<type>[;endpoint=<endpoint>]

store=json-file;path=<path>
