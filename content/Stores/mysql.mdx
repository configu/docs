...
Integrates the Configu Orchestrator with a MySQL database.
...
View additional configuration options [here](https://typeorm.io/data-source-options#mysql--mariadb-data-source-options).

### Test command

```bash
configu test --store "mysql-store" --clean
```

### Upsert command

```bash
configu upsert --store "mysql-store" --set "test" --schema "./get-started.cfgu.json" \
    -c "GREETING=hey" \
...
```bash
configu eval --store "mysql-store" --set "test" --schema "./get-started.cfgu.json" \
 | configu export
```
...
```bash
configu delete --store "mysql-store" --set "test" --schema "./get-started.cfgu.json"
```
