...
(async () => {
  try {
    const store = new MariaDBConfigStore({
...
```bash
configu test --store "mariadb-store" --clean
```

### Upsert command

```bash
configu upsert --store "mariadb-store" --set "test" --schema "./get-started.cfgu.json" \
    -c "GREETING=hey" \
...
```bash
configu eval --store "mariadb-store" --set "test" --schema "./get-started.cfgu.json" \
 | configu export
```
...
```bash
configu delete --store "mariadb-store" --set "test" --schema "./get-started.cfgu.json"
```
