  ...
  try {
    const store = new AzureKeyVaultConfigStore({
      credential: {
        ...
        workloadIdentityClientId: 'example-workloadIdentityClientId',
      },
...
TODO
Configu's CLI needs to be authorized to access Azure Key Vault. This can be done using the [default azure credentials](https://www.npmjs.com/package/@azure/identity#defaultazurecredential) which by default will read account information specified via [environment variables](https://www.npmjs.com/package/@azure/identity#environment-variables) and use it to authenticate.. The vault url must be provided via the [.configu file](../cli-config).
